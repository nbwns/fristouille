<template>
	<div
		class="w-fit rounded-3xl flex justify-start items-center hover:bg-secondary/50 px-4 py-2 transition-all duration-300 group">
		<button @click="toggleDarkMode" class="flex space-x-2 items-center justify-start w-full cursor-pointer">
			<span class="text-base select-none text-label group-hover:text-accent transition-colors duration-300">Thème</span>
			<div class="flex items-center">
				<svg v-if="dark" xmlns="http://www.w3.org/2000/svg"
					class="h-6 w-6 text-label group-hover:text-accent transition-colors duration-300" viewBox="0 0 20 20"
					fill="currentColor">
